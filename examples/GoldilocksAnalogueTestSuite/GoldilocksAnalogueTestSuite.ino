...
void setup() {
  ...
  xTaskCreate(TaskReport, (const portCHAR *) "RedLED", 256, NULL, 2, NULL);
  xTaskCreate(TaskAnalogue, (const portCHAR *) "Analogue", 256, NULL, 1,  NULL );
...
static int8_t testSPIEEPROM( uint_farptr_t p1, uint16_t p2 ) {
  ...
  if (ReturnCode) 
    return ReturnCode; // problem with opening the EEPROM / SRAM
  uint_farptr_t FarAddress = p1;
  ReturnCode = SPIRAM_write( FarAddress, Buff, (size_t)p2);
  if (ReturnCode) 
    return ReturnCode;   /* error or disk full */
  for (uint16_t i = 0; i < p2; ++i) {
    uint8_t read_result;
